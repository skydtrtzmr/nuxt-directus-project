...
async function selectDeterministicOptions(
    ...
    switch (questionType) {
        case "q_mc_single": {
            ...
            if (optionLabel) {
                console.log(`自动化测试：[单选题] 在题目元素中选择: ${targetChar}`);
                optionLabel.click();
                await delay(200); 
            } else {
        ...
        case "q_mc_binary": { 
            ...
            const optionLabel = getOptionLabel(targetChar);
            if (optionLabel) {
                console.log(`自动化测试：[判断题] 在题目元素中选择: ${targetChar}`);
                optionLabel.click();
                await delay(200);
            } else {
        ...
        case "q_mc_multi": { 
            ...
            const currentPattern = patterns[counters.q_mc_multi % patterns.length];
            console.log(`自动化测试：[多选题] 在题目元素中选择模式: ${currentPattern.join(', ')}`);
            for (const char of currentPattern) {
                ...
                if (optionLabel) {
                    ...
                    console.warn(`自动化测试：[多选题] 未在以下元素中找到选项 ${char}:`, questionElement);
                }
            }
            counters.q_mc_multi++;
            break;
        ...
        case "q_mc_flexible": { 
            const patterns = [['A', 'B'], ['B', 'C'], ['C', 'D'], ['D', 'A']];
            const currentPattern = patterns[counters.q_mc_flexible % patterns.length];
            console.log(`自动化测试：[不定项选择题] 在题目元素中选择模式: ${currentPattern.join(', ')}`);
            for (const char of currentPattern) {
                const optionLabel = getOptionLabel(char);
                if (optionLabel) {
                    ...
                    console.warn(`自动化测试：[不定项选择题] 未在以下元素中找到选项 ${char}:`, questionElement);
                }
            }
...
export async function runCompleteExamScenario(
    ...
    while (true) {
        ...
        if (nextQuestionButton && nextQuestionButton.offsetParent !== null && !nextQuestionButton.disabled) {
            ...
            const endDialogMessageElement = Array.from(document.querySelectorAll(".p-dialog-content span")).find(
                span => span.textContent?.trim() === "当前已经是最后一题！"
            );
            
            if (endDialogMessageElement) {
                const dialogElement = endDialogMessageElement.closest(".p-dialog");
                if (dialogElement) {
                    const confirmButton = dialogElement.querySelector("button[label='确定']") as HTMLButtonElement | null;
                    if (confirmButton) {
                        console.log("自动化测试：检测到'已经是最后一题'提示框，点击确定并准备交卷。");
                        confirmButton.click();
                        await delay(500);
                        break; 
                    }
    ...
    await delay(1000); 
    const confirmSubmitDialog = Array.from(document.querySelectorAll(".p-dialog-header-title")).find(
        header => header.textContent?.trim() === "警告"
    )?.closest(".p-dialog");

    if (confirmSubmitDialog) {
        const confirmSubmitButton = confirmSubmitDialog.querySelector("button[label='确定'][severity='danger']") as HTMLButtonElement | null;
        ...
        console.log("自动化测试：未检测到确认提交试卷对话框，可能直接提交或无需确认。");
    }
